...
1. 오늘 작업하면서 막힌 부분이 있었나요?
2. 내일 아침 회의에서 팀원에게 확인할 사항이 있나요?
3. 이번 주 주간보고에 포함할 성과가 있나요?
4. 내일 우선적으로 처리해야 할 작업이 있나요?

## 2단계: 문서 업데이트

답변을 바탕으로:

1. `.context/progress.md` 업데이트
   - 완료한 작업, 진행 중인 작업
   - 내일 우선순위 (P0/P1)
   - 팀원 확인 필요 사항
   - 주간보고 포함 항목

2. `.context/daily/$(date +%Y-%m-%d).md` 생성 (선택)
   - 상세 이슈, 의사결정 맥락
...
1. `.context/daily/` 폴더에서 이번 주 파일들 읽기
2. `.context/progress.md`에서 주간보고 항목 확인
...
`.context/` 폴더를 `.gitignore`에 추가하고 다음과 같이 구성합니다:
...
```
.context/
├── progress.md              # 현재 진행 상황 (Shrivu 스타일)
├── daily/                   # 일별 상세 로그 (선택)
│   └── 2025-01-17.md
└── decisions.md             # 의사결정 기록 (선택)
```

Shrivu의 `PROGRESS.md`처럼 단일 파일로 현재 상태를 관리하고, 필요시 일별 로그를 추가합니다.
...
## 다음 (우선순위)
- [ ] **P0**: Target 추가 기능
- [ ] **P1**: Load Balancing 테스트

## 팀 협업
- **확인 필요**: DB 스키마 변경 건 백엔드팀 협의
- **주간보고**: Gateway MVP 완료 (1/15)
...
# 퇴근 전
/daily-end            # AI가 질문 → 진행상황/팀 협업사항 저장

# 금요일 (주간보고 전)
/weekly-report        # 주간보고 형식으로 정리
```
