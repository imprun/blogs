...
실무에서는 Claude가 주력 구현 에이전트이지만, 최근에는 Codex와 Gemini를 혼합해서 사용하고 있습니다. 각 모델이 서로 상호보완해 주는 효과가 있습니다.

Gemini는 아직 경험치가 많지 않아 구체적인 평가는 어렵지만, Codex는 엔터프라이즈급 소프트웨어 개발 시 Claude가 작성한 코드를 리뷰하거나 아키텍처 문서를 검토할 때 의미 있는 인사이트를 제공합니다. 단독으로 문서 생성 능력은 Claude에 못 미치지만, 리뷰어 역할에서는 Claude가 놓칠 수 있는 관점을 짚어주는 경우가 많아 프로젝트에서 함께 활용하고 있습니다.
